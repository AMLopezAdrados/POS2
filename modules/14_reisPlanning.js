  ...
  _modal(()=>{
    ...
    events.forEach(entry => {
      ...
      orderedProducts.forEach(name => {
        ...
        planPieces[name] = direct || stored || 0;
      });
      ...
      orderedProducts.forEach(name => {
        ...
        const applyValue = (nextValue) => {
          ...
          piecesLabel.textContent = pieces ? `${pieces} stuks` : '0 stuks';
...
function _wizardVoorraadStap3(aStart, aEnd, omzetData){
  ...
  plannedEvents.forEach(evt => {
    ...
    if (!eventsByBus[busKey]) {
      eventsByBus[busKey] = { bus: busKey, events: [], required: {} };
    }
    eventsByBus[busKey].events.push(evt);
    Object.entries(evt.plan).forEach(([product, qty]) => {
      const amount = Math.max(0, Math.round(Number(qty)||0));
      if (!amount) return;
      ...
      eventsByBus[busKey].required[product] = (eventsByBus[busKey].required[product]||0) + amount;
    });
  });
  ...
  const busPlans = Object.values(eventsByBus).map(plan => {
    ...
    Object.entries(plan.required).forEach(([product, neededRaw]) => {
      const needed = Math.max(0, Math.round(Number(neededRaw)||0));
      if (!needed) return;
      const availableBus = Math.max(0, Number(busStock[product]||0));
      const takeFromBus = Math.min(needed, availableBus);
      if (takeFromBus) {
        busStock[product] = availableBus - takeFromBus;
      }

      let remainder = needed - takeFromBus;
      const availableCentral = Math.max(0, Number(centralStock[product]||0));
      const fromCentralExisting = Math.min(remainder, availableCentral);
      if (fromCentralExisting) {
        centralStock[product] = availableCentral - fromCentralExisting;
      }
      remainder -= fromCentralExisting;

      let fromOrder = 0;
      if (remainder > 0) {
        fromOrder = remainder;
        orderNeeds[product] = (orderNeeds[product]||0) + fromOrder;
        remainder = 0;
      }
      ...
      load[product] = {
        needed,
        fromBus: takeFromBus,
        fromCentral: fromCentralExisting,
        fromOrder,
        capacity: productMeta[product]?.capacity || 1
      };
    });
  ...
  _modal(()=>{
    ...
    busPlans.sort((a,b)=> a.bus.localeCompare(b.bus)).forEach(plan => {
      ...
      eventsList.className='rp-pack-events';
      eventsList.textContent = plan.events.map(ev => `${ev.naam || 'Event'} (${_fmt(ev.start)} → ${_fmt(ev.eind)})`).join(' · ');
...
function _injectPlannerCSS(){
  ...
  .rp-crate-right{ display:flex; flex-direction:column; align-items:flex-end; gap:.2rem; }
  ...
  .rp-crate-pieces{ font-size:.75rem; color:#25633b; font-weight:700; }
  ...
  .rp-pack{ border:1px solid #e5e7eb; border-radius:12px; padding:.6rem .75rem; margin:.6rem 0; background:#f9fafb; }
  .rp-pack-head{ font-weight:900; color:#194a1f; margin-bottom:.25rem; }
  .rp-pack-events{ font-size:.85rem; color:#4b5563; margin-bottom:.5rem; }
